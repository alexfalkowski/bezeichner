...
When('I request identifiers with gRPC:') do |table|
  ...
  @response = Bezeichner::V1.server_grpc.generate_identifiers(request, { metadata: metadata })
...
When('I request to map identifiers with gRPC:') do |table|
  ...
  @response = Bezeichner::V1.server_grpc.map_identifiers(request, { metadata: metadata })
rescue StandardError => e
  @response = e
end
